    ...
    <form action="{{ route('control.banks.update', ['bank' => $bank->id]) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="row">
        <div class="mb-3 col-12 col-lg-6">
          ...
          <input type="text" class="form-control @error("title") is-invalid @enderror" id="title" name="title" value="{{ old('title') ? old('title') : $bank->title }}">
          @error('title')
        ...
        <div class="mb-3 col-12 col-lg-6">
          ...
          <input type="date" name="date" id="date" class="form-control" value="{{ old('date') ? old('date') : $bank->date }}">
          @error('date')
            <div class="invalid-feedback">
              {{ $message }}
            </div>
